...
├── overlays/                       # Jira mappings per submodule
│   ├── backend/
...
4. Creates `overlays/backend/` directory
...
mkdir -p overlays/backend
mkdir -p overlays/frontend
...
2. Applies Jira mappings from `overlays/` (if any)
...
**Field descriptions**:
- `name`: Identifier for submodule (used in overlay paths: `overlays/<name>/`)
...
**Overlay file structure**: `overlays/<submodule-name>/<feature-id>.md`

**Example**: `overlays/backend/CLI-CORE.md`
```markdown
---
feature: CLI-CORE
---
...
- [ ] Overlays directory exists: `overlays/`
- [ ] Overlay subdirectories exist: `overlays/<name>/` for each submodule
