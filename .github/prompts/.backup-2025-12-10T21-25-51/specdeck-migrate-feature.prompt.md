...
**Target path**: `overlays/backend/CLI-CORE.md`
...
- Path must match submodule name: `overlays/<submodule>/`
...
2. Applies Jira mappings from `overlays/backend/CLI-CORE.md`
...
# Overlay exists
[ -f overlays/backend/CLI-CORE.md ] && echo "✓ Overlay created"
```
...
grep "PROJ-" overlays/backend/CLI-CORE.md
...
- Overlay: `overlays/backend/` (not `overlays/CLI-CORE.md`)
...
After migration:
  ...
  overlays/backend/CLI-CORE.md (covers both)
```
...
### 3. Remove Overlay
```bash
rm overlays/backend/CLI-CORE.md
```
...
3. Write: overlays/backend/CLI-CORE.md
          └→ Jira mappings only
