...
- **And** `.specdeck-version` file is created or updated with the Copilot target

#### Scenario: User initializes Windsurf workflows
- **Given** user is in a SpecDeck project directory
- **When** user runs `specdeck init windsurf`
- **Then** `.windsurf/workflows/` directory is created
- **And** workflow files are copied from CLI bundle
- **And** AGENTS.md is updated with SpecDeck section
- **And** `.specdeck-version` file is created or updated with the Windsurf target

#### Scenario: Idempotent initialization
- **Given** Copilot templates are already installed
...
The CLI MUST copy all prompt template files from bundled resources to assistant-specific directories.

#### Scenario: All templates are installed
- **Given** CLI has bundled template files
- **When** user runs `specdeck init copilot`
- **Then** all templates are copied to `.github/prompts/`
- **And** file permissions are preserved
- **And** file encoding is UTF-8

#### Scenario: Windsurf workflows are installed
- **Given** CLI has bundled template files
- **When** user runs `specdeck init windsurf`
- **Then** all workflows are copied to `.windsurf/workflows/`
- **And** workflow filenames match prompt base names with `.md` extension
- **And** file encoding is UTF-8

#### Scenario: Directory creation
- **Given** `.github/prompts/` or `.windsurf/workflows/` does not exist
...
- **When** user runs `specdeck init copilot` or `specdeck init windsurf`
- **Then** AGENTS.md is created in project root
- **And** file contains `<!-- SPECDECK:START -->` managed block
- **And** block summarizes SpecDeck structure and commands
