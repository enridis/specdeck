...
## Coordinator Conflict Handling (Recommended)

**Principle**: Coordinator sync is cache-only (submodules → coordinator). Do not write back to submodules unless explicitly requested by the PM.

**Stop-and-ask conflicts** (do not auto-resolve):
- Duplicate Story IDs across submodules
- Overlay references a Story ID that does not exist in any submodule
- Conflicting Jira mappings for the same Story ID across overlays
- Same Release ID used with different title/timeframe/objectives across submodules (if comparing releases)

**Resolution flow**:
1. Report the conflicting IDs and repos.
2. Ask the PM which repo or mapping is the source of truth.
3. Apply the PM decision in the correct submodule or overlay.
4. Run `specdeck sync` again to refresh the cache.

**LLM guardrails**:
- Never merge or overwrite submodule data automatically.
- If conflict detected, stop and ask for PM direction before continuing.
